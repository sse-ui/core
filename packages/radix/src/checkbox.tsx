...
function CheckboxProvider<State extends CheckedState = CheckedState>(
  ...
  const [bubbleInput, setBubbleInput] = React.useState<HTMLInputElement | null>(
    null
  );
  ...
  (
    {
      ...
      onClick,
      ...checkboxProps
    }: ScopedProps<CheckboxTriggerProps>,
    forwardedRef
  ) => {
    const {
      control,
...
>(
  (
    { __scopeCheckbox, ...props }: ScopedProps<CheckboxBubbleInputProps>,
    forwardedRef
  ) => {
    const {
      control,
