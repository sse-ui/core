...
import { unstable_createCollection as createCollection } from "./collection";
...
const [
  Collection,
  { useCollection, createCollectionScope, useInitCollection },
] = createCollection<HTMLInputElement>(ONE_TIME_PASSWORD_FIELD_NAME);
const [createOneTimePasswordFieldContext] = createContextScope(
...
const [OneTimePasswordFieldContext, useOneTimePasswordFieldContext] =
  createOneTimePasswordFieldContext<OneTimePasswordFieldContextValue>(
    ONE_TIME_PASSWORD_FIELD_NAME
  );
...
>(function OneTimePasswordFieldHiddenInput(
  {
    __scopeOneTimePasswordField,
    ...props
  }: ScopedProps<OneTimePasswordFieldHiddenInputProps>,
  forwardedRef
) {
...
>(function OneTimePasswordFieldInput(
  ...
  const lastSelectableIndex = clamp(totalValue.length, [
    0,
    collection.size - 1,
  ]);
                ...
                switch (event.key) {
                  ...
                  case "Backspace": {
                    ...
                    if (currentValue === "") {
                      ...
                      const isClearing =
                        event.key === "Clear" || event.metaKey || event.ctrlKey;
                      if (isClearing) {
                        dispatch({ type: "CLEAR", reason: "Backspace" });
                      } else {
